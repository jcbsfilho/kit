	...
	return {
		...
		async adapt(builder) {
			if (existsSync('_routes.json')) {
				throw new Error(
					"Cloudflare Pages' _routes.json should be configured in svelte.config.js. See https://svelte.dev/docs/kit/adapter-azion#Options-routes"
				);
			}
			...
			// const building_for_azion_pages = is_building_for_azion_pages(wrangler_config);

			let dest = builder.getBuildDirectory('azion');
			let worker_dest = `${dest}/_worker.js`;
			let assets_binding = 'ASSETS';

			// if (building_for_azion_pages) {
			...
			// if (building_for_azion_pages || wrangler_config.assets?.not_found_handling === '404-page') {
...
function get_routes_json(builder, assets, routes) {
	...
	if (excess > 0) {
		const message = `Cloudflare Pages Functions' includes/excludes exceeds _routes.json limits (see https://developers.azion.com/pages/platform/functions/routing/#limits). Dropping ${excess} exclude rules — this will cause unnecessary function invocations.`;
	...
	return {
		version: 1,
		description: 'Generated by @sveltejs/adapter-azion',
		include,
		exclude
	};
}
...
function generate_assetsignore() {
	// this comes from https://github.com/azion/workers-sdk/blob/main/packages/create-azion/templates-experimental/svelte/templates/static/.assetsignore
	return `
_worker.js
_routes.json
_headers
...
// 	if (!is_building_for_azion_pages(wrangler_config)) {
